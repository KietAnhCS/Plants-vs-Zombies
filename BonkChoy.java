...
public class BonkChoy extends Plant {
    ...
    private void handleNormalCombat() {
        ...
        if (!targets.isEmpty()) {
            ...
            int dmg;
            if (beingEaten) {
                dmg = isKO ? PlantRegistry.BONK_DMG_KO + 5 : PlantRegistry.BONK_DMG_NORMAL + 5;
            } else {
                dmg = isKO ? PlantRegistry.BONK_DMG_KO : PlantRegistry.BONK_DMG_NORMAL;
            }
            applyDmg(targets, PlantRegistry.BONK_ATTACK_DELAY, dmg, isKO);
        } else {
            playLoop(idle, 40);
