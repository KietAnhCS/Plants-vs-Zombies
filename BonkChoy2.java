...
public class BonkChoy2 extends Plant {
    ...
    private void handleNormalCombat() {
        ...
        if (!targets.isEmpty()) {
            ...
            int dmg;
            if (beingEaten) {
                dmg = isKO ? PlantRegistry.BONK2_DMG_KO : PlantRegistry.BONK2_DMG_NORMAL + 5;
            } else {
                dmg = isKO ? PlantRegistry.BONK2_DMG_KO : PlantRegistry.BONK2_DMG_NORMAL;
            }
            
            applyDmg(targets, PlantRegistry.BONK2_ATTACK_DELAY, dmg, isKO);
        } else {
            playLoop(idle, 40);
