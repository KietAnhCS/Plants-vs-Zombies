...
public class Buckethead extends Zombie {
    ...
    public Buckethead() {
        ...
        wNormal  = importSprites(SpriteKey.BUCKET_WALK.path,          7);
        wD1      = importSprites(SpriteKey.BUCKET_WALK_D1.path,       7);
        wD2      = importSprites(SpriteKey.BUCKET_WALK_D2.path,       7);
        wBare    = importSprites(SpriteKey.SHARED_WALK_BARE.path,     7);
        wArmless = importSprites(SpriteKey.SHARED_WALK_ARMLESS.path,  7);
        eNormal  = importSprites(SpriteKey.BUCKET_EAT.path,           7);
        eD1      = importSprites(SpriteKey.BUCKET_EAT_D1.path,        7);
        eD2      = importSprites(SpriteKey.BUCKET_EAT_D2.path,        7);
    ...
    public void hit(int dmg) {
        ...
        AudioManager.playSound(80, false,
            bucket ? "shieldhit.mp3"  : "splat.mp3",
            bucket ? "shieldhit2.mp3" : "splat2.mp3");
        if (isLiving()) {
            if      (hp > ZombieRegistry.BUCKET_D1)    hitFlash(eating ? eNormal  : wNormal,
                                                                eating ? SpriteKey.BUCKET_EAT.path          : SpriteKey.BUCKET_WALK.path);
            else if (hp > ZombieRegistry.BUCKET_D2)    hitFlash(eating ? eD1      : wD1,
                                                                eating ? SpriteKey.BUCKET_EAT_D1.path       : SpriteKey.BUCKET_WALK_D1.path);
            else if (hp > ZombieRegistry.BUCKET_BARE)  hitFlash(eating ? eD2      : wD2,
                                                                eating ? SpriteKey.BUCKET_EAT_D2.path       : SpriteKey.BUCKET_WALK_D2.path);
